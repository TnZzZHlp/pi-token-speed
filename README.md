...
`pi-token-speed` is a [Pi](https://github.com/earendil-works/pi-mono) extension that keeps the model's average token output speed in the footer as a single value.

While a response streams, the footer shows the running average speed of the current message, for example `17.3 tok/s`. When the response finishes, the exact token count from the provider replaces the estimate and the final value stays in the footer until the next response starts.
...
- `message_update` deltas (text, thinking, and tool-call fragments) are counted as they arrive. Tokens are estimated at 4 characters per token while streaming, and the footer shows the running average speed of the current message.
- At `message_end` the estimate is replaced by the provider-reported `usage.output` token count. The final value stays in the footer until the next response starts.
