...
test("streaming turn shows running average and keeps it after completion", async () => {
	...
	try {
		...
		// Live footer shows the running average speed and elapsed time.
		const live = harness.statuses.at(-1);
		assert.equal(live.key, STATUS_KEY);
		assert.equal(live.text, "22 tok/s | 1.8s");
	...
	assert.equal(final.text, "1600 tok/s | 1.8s");
	...
	assert.match(harness.notifications[0], /Last: 1600 tok\/s \| 1\.8s/);
...
test("thinking deltas are labeled in the running average", () => {
	...
	const live = harness.statuses.at(-1);
	assert.equal(live.key, STATUS_KEY);
	assert.equal(live.text, "21 tok/s | 1.0s (thinking)");
